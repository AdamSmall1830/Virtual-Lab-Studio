...
- When regenerating the spec from the FastAPI app, strip the `/api` path prefix (orval `baseUrl` already adds it — keeping both yields `/api/api/...` 404s on every client call) and drop query params (TS2308 collision).
