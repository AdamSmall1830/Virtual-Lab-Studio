...
- Replit AI zero-key is owner-billed, so it is gated by the REPLIT_AI_ALLOWED_EMAILS allowlist (empty = nobody); both the environment endpoint and provider creation enforce it. **Why:** other signed-in users must never spend the owner's Replit credits.
